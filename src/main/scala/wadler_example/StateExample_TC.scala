...
object StateExample_TC {
  ...
  implicit object StateMonad extends Monad[State] {
    ...
    override def map[A, B](fa: State[A])(f: A => B): State[B] = {
      val function : Int => (B, Int) = (x: Int) => {
        val (a, y) : (A, Int) = fa(x)
        val (b, z) : (B, Int) = State(_ => (f(a),y))(y)
        (b, z)
      }
      State(function)
    }
  ...
  lazy val withMonads: Unit = {
    ...
    implicit val f: (Int, Int, Term) => State[Int] = (a: Int, b: Int, _) => tick.bind(_ => StateMonad.unit(a / b))
  ...
  trait StateGen[A, S]
  case class State[A](func: Int => (A, Int)) extends StateGen[A, Int] {
