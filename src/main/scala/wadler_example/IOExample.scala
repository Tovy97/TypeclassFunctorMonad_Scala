...
object IOExample {
  ...
  lazy val withMonads: Unit = {
    ...
    implicit val f: (Int, Int, Term) => IO[Int] = (a: Int, b: Int, e: Term) => out(line(e, a / b)).bind(Unit => IOMonad.unit(a / b))
    implicit val g: (Int, Term) => IO[Int] = (a: Int, e: Term) => out(line(e, a)).bind(Unit => IOMonad.unit(a))
    try {
